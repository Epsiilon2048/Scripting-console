...
function new_object_editor(object){

var editor = new Console_dock()

var name_box = new_text_box(undefined, undefined)
name_box.initial_ghost_text = "Object"
name_box.draw_name = false
name_box.att.length_min = 10
global._scrvar_ = {name_box, editor}
name_box.on_input = function(){
	static name_box = global._scrvar_.name_box
	static editor = global._scrvar_.editor
	
	var ind = asset_get_index(name_box.text)
	if not object_exists(ind)
	{
		name_box.att.text_color = dt_unknown
		exit
	}
	
	editor.association = ind
	name_box.att.text_color = dt_instance
}

var id_box = new_display_box("id", "id", false)
id_box.allow_printing = false
id_box.att = id_box.att
...
var variable_dock = new_variable_display()
